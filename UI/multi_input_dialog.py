...
class MultiInputDialog(SmallPopup):
    """
    An input pop-up window which gets and returns multiple inputs from the user (text)
    as a dictionary. The input widgets can be either entries or dropdown menus (ComboBox)
    get_user_input() should be called after creating a MultiInputDialog so that the input
    is returned to that function
    """

    def __init__(self, input_field_names, dropdown_menu_options_list, subject_name):
        """
        input_field_names is a list of the text that each label for each input will
        display.
        ...
        subject_name is the type of thing that is being added e.g. Design, Product Type
            etc.
        """
        super().__init__()
    ...
    def display_widgets(self, input_field_names, dropdown_menu_options_list):
        ...
        for row, input_row_info in enumerate(
            zip(input_field_names, dropdown_menu_options_list)
        ):
            ...
            if dropdown_menu_options is None:
                ...
                input_widget = customtkinter.CTkComboBox(
                    self, values=dropdown_menu_options
                )
        ...
        add_button = customtkinter.CTkButton(
            self, text=f"Add {self.subject_name}", command=self.on_add_button_click
        )
    ...
    def get_user_input(self):
        ...
        Returns None if the window is closed by the user, otherwise returns a dictionary
        of the user inputs
        """
