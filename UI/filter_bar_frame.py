...
class FilterBarFrame(customtkinter.CTkFrame):
    """
    A frame containing the widgets which the user can use to filter the items being
    displayed
    """
    ...
    def __init__(
        ...
        sub_type_options,
        colour_options,
        product_component_switch_toggle_callback
    ):
        ...
        # add the empty option to the dropdown menus as the default, and order the list
        # alphabetically
        prepare_dropdown_options_lists(
            [design_options, theme_options, type_options, sub_type_options, colour_options]
        )
        ...
        self.product_component_switch = customtkinter.CTkSegmentedButton(
            self, values=product_component_vals
        )
        self.product_component_switch.set(product_component_vals[0])
        self.add_labelled_input_widget(
            product_component_lbl_text, self.product_component_switch
        )
        ...
        self.design_theme_dropdown = customtkinter.CTkOptionMenu(
            self, values=theme_options
        )
        ...
        # initialise the filter state dictionary - to be used to check if the filters
    ...
    def add_label(self, lbl_widget):
        ...
        lbl_widget.grid(
            row=0,
            column=self.current_widget_num,
            padx=(config.WIDGET_X_PADDING,
            config.WIDGET_X_PADDING/3),
    ...
    def add_input_widget(self, widget):
        """
        Adds an input widget to the layout grid, with the command to update the results
        whenever a change is made
        """
        widget.grid(
            row=0,
            column=self.current_widget_num,
            padx=(0, config.WIDGET_X_PADDING),
            pady=config.WIDGET_Y_PADDING
        )
        self.current_widget_num += 1
    ...
    def on_filter_widget_update(self, event):
        """
        Function to be run when any of the filter widgets have been updated, regardless
        of whether their values have actually been changed
        """
        ...
        if new_filter_values != self.current_filter_values:
            # updates table after applying new filters and keeps
            # self.current_filter_values up-to-date
            ...
            # callback function disable ViewItemsFrame buttons after product/component
            # switch is toggled
    ...
    def add_command_to_widget(self, widget):
        ...
        # if the widget is a CTkEntry then the command needs to be added differently to
        # other widgets
    ...
    def change_product_widgets_state(self, new_state):
        """
        Sets whether the widgets that are used for filtering just products (not
        components) should be greyed out or not
        """
        # widgets that shouldn't be greyed out ever
        component_compatible_widgets = [
            self.product_component_switch, self.name_entry
        ]
    ...
    def display_components(self):
        ...
        # forces the components to be displayed, as setting the switch doesn't do that
        self.on_filter_widget_update(None)
